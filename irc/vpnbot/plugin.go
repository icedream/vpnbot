...
func New(b bot.Bot, whoisPlugin *whois.Plugin, tempbanPlugin *tempban.Plugin, nickservPlugin *nickserv.Plugin) *Plugin {
	if whoisPlugin == nil {
	...
	b.Commands().Add("globalban", bot.Command{
		...
		Handler: func(e *bot.Event) {
			...
			plugin.banGlobal(nick, banmask, reason, duration)
		},
	})
			...
			func(conn *client.Conn, line *client.Line) {
				...
				if len(line.Args) > 0 {
					...
					case "Excess flood":
						...
						plugin.banGlobal(line.Nick, banmask,
